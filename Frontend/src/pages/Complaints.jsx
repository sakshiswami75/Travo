        ...
        <>
          ...
          {step === 'preview' && (
            <main className="flex-1 w-full max-w-md mx-auto p-margin-mobile flex flex-col pt-6 pb-24 overflow-y-auto">
              ...
              {aiResult?.aiAnalysis && !uploading && !detecting && (
                <div className="bg-surface-container-lowest border border-primary/30 shadow-[0_4px_16px_rgba(124,58,237,0.1)] rounded-2xl p-5 mb-6 relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1 h-full bg-primary" />
                  <div className="flex items-center gap-2 mb-4 border-b border-outline-variant/20 pb-3">
                    <span className="material-symbols-outlined text-primary text-xl">auto_awesome</span>
                    <h3 className="text-sm font-black text-on-surface tracking-wider uppercase">AI Hazard Analysis</h3>
                    <span className="ml-auto text-[10px] bg-primary/10 text-primary px-2 py-0.5 rounded-full font-bold">{aiResult.aiAnalysis.confidence}% Match</span>
                  </div>
                  
                  <div className="space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-on-surface-variant font-bold uppercase tracking-wider">Detected</span>
                      <span className="text-sm font-black text-on-surface">{aiResult.aiAnalysis.detectedHazard}</span>
                    </div>
                    
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-on-surface-variant font-bold uppercase tracking-wider">Risk Level</span>
                      <span className={`text-xs font-black uppercase px-2 py-1 rounded-md ${
                        aiResult.aiAnalysis.riskLevel.includes('Critical') ? 'bg-error-container text-error' :
                        aiResult.aiAnalysis.riskLevel.includes('High') ? 'bg-tertiary-container text-tertiary' :
                        aiResult.aiAnalysis.riskLevel.includes('Medium') ? 'bg-orange-500/20 text-orange-600' :
                        'bg-secondary-container text-secondary'
                      }`}>{aiResult.aiAnalysis.riskLevel}</span>
                    </div>

                    <div className="flex justify-between items-center">
                      <span className="text-xs text-on-surface-variant font-bold uppercase tracking-wider">Safety Score</span>
                      <span className="text-sm font-black text-on-surface">{aiResult.aiAnalysis.roadSafetyScore}/100</span>
                    </div>

                    <div className="flex justify-between items-center">
                      <span className="text-xs text-on-surface-variant font-bold uppercase tracking-wider">Damage Prob.</span>
                      <span className="text-sm font-black text-on-surface">{aiResult.aiAnalysis.vehicleDamageProbability}</span>
                    </div>

                    <div className="mt-4 pt-3 border-t border-outline-variant/20">
                      <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-wider block mb-1">Suggested Action</span>
                      <p className="text-sm font-medium text-on-surface italic">{aiResult.aiAnalysis.suggestedAction}</p>
                    </div>
                  </div>
                </div>
              )}
